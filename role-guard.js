  ...
  function showPaywall(role) {
    ...
    if (isGuest) {
      ...
      overlay.innerHTML = ''
        ...
        // Google
        + '  <button onclick="window.signInWithGoogle&&window.signInWithGoogle()" style="width:100%;padding:14px;background:#fff;border:none;border-radius:12px;color:#1f2937;font-size:15px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;transition:all .2s;box-shadow:0 4px 16px rgba(0,0,0,.25);font-family:inherit;"'
        ...
        + '    Войти / Зарегистрироваться через Google'
        + '  </button>'
        + '  <p style="font-size:11px;color:#475569;margin-bottom:16px;">Один клик — вход и регистрация одновременно</p>'
        // Divider
