 ...
 *
 * `translate`, `transcribe` и `robotics` отвергаются, хотя формально умеют
 * вебсокет: live-translate переводит речь вместо того, чтобы вести разговор,
 * transcribe-live только слушает и на AUDIO-ответ закрывает сокет кодом 1007,
 * а robotics обучена под управление механикой. Все прошли бы по методу и по
 * номеру поколения — и все сломали бы звонок. transcribe-live уже сломала:
 * 3.5 обыграла 3.1-flash-live по поколению, и Live не поднимался ни разу.
 */
const NEVER = [
  'tts',
  'image',
...
export const MODEL_RULES: Record<ModelKind, ModelRule> = {
   ...
   * встаёт на торге.
   *
   * Раньше полный flash шёл первым как «качественнее». Живой звонок с сайта
   * 06.09.2026: gemini-3.6-flash — рассуждающая модель, 19–57 секунд на ход,
   * 503 «high demand» и 20 запросов в сутки — квота кончилась за один
   * вечер. Для телефонного разговора это «брокер думает…» на минуту. Замер
   * flash-lite на той же цепочке: 0,6–0,8 с, инструменты держит, 500 в сутки.
   * Поэтому lite первой, полный flash — запасной.
   */
  chat: {
    method: 'generateContent',
