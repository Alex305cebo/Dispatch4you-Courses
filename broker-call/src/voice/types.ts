...
export interface TransportDeps {
  ...
  seed: string
  /**
   * Откуда брать сессию Gemini Live. На боевом это Cloudflare Worker — PHP
   * вебсокет держать не умеет. Пусто — тот же сервер, что и остальные ручки.
   */
  geminiSessionUrl?: string
  /**
   * Что брокер уже знает, одной строкой — уходит с каждым ходом. История в
   * модель идёт обрезанной, и без этого брокер переспрашивал MC, названный
   * первой фразой.
   */
  knownFacts?: () => string
  ...
  direction?: string
  /** Модель распознавания. Приходит с сервера: имя в бандле уже стоило нам трёх поломок. */
  sttModel?: string
