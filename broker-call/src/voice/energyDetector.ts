...
export const DEFAULT_ENERGY_OPTIONS: EnergyDetectorOptions = {
  ...
  //
  // 450 мс — граница, ниже которой начинает резать людей на середине фразы.
  // Раньше стояло 700 «с запасом», но запас брался из воздуха, а стоил четверти
  // секунды в каждом обмене репликами. Оставшийся риск закрывает короткий
  // отклик брокера: даже сорвавшись рано, он отзывается «угу», а не начинает
  // говорить поверх.
  hangoverMs: 450,
  minSpeechMs: 320,
  floorFloor: 0.0015,
