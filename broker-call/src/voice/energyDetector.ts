...
export const DEFAULT_ENERGY_OPTIONS: EnergyDetectorOptions = {
  ...
  //
  // 600 мс — компромисс: заметно быстрее прежних 700, но фразу уже не рвёт.
  // Ощущаемую задержку всё равно закрывает короткий отклик брокера, поэтому
  // экономить здесь дальше нет смысла — цена ошибки выше выигрыша.
  hangoverMs: 600,
  minSpeechMs: 320,
  floorFloor: 0.0015,
  warmupMs: 500,
