...
export const DEFAULT_ENERGY_OPTIONS: EnergyDetectorOptions = {
  ...
  endFactor: 1.8,
  attackFrames: 2,
  // Эта пауза платится на КАЖДОЙ реплике и была самым медленным звеном всего
  // звонка — медленнее и распознавания, и самой модели.
  //
  // 450 мс оказались слишком жадными: живая речь резалась на куски —
  // «Freight,» и «Hi.» уходили в распознавание отдельными репликами. Человек
  // делает паузы внутри фразы, особенно на неродном языке, и это нормально.
  //
  // 600 мс тоже рвали: с живого звонка на сайте — «текст не пишется
  // полностью», фраза на неродном языке уходила в распознавание кусками.
  // 900 мс: пауза «подумать над словом» прощается, ответ брокера всё равно
  // дольше. Цена ошибки здесь выше выигрыша в скорости.
  hangoverMs: 900,
  minSpeechMs: 320,
  floorFloor: 0.0015,
  warmupMs: 500,
