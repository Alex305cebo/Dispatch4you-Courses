/**
 * onboarding.js v3 — Пошаговый тур для новых пользователей
 * Desktop: открывает dropdown в навбаре и указывает на элементы
 * Mobile: открывает гамбургер-меню, скроллит к элементу, tooltip снизу экрана
 */
(function () {
  'use strict';

  var STORAGE_KEY = 'onboarding_done_v6';
  ...
  function openMobMenu() {
    ...
    if (menu) { menu.classList.add('active'); menu.style.zIndex = '999993'; }
    if (overlay) { overlay.classList.add('active'); overlay.style.zIndex = '999990'; }
    document.body.style.overflow = 'hidden';
  }
  function openMobAccordion(index) {
    // Закрываем все, открываем нужный
    document.querySelectorAll('.mob-acc').forEach(function (a) { a.classList.remove('open'); });
    var accs = document.querySelectorAll('.mob-acc');
    if (accs[index]) accs[index].classList.add('open');
  }
  ...
  var STEPS = [
    ...
    // 2. Курс обучения — открываем первый dropdown
    {
      desktopTarget: '.nav-item:nth-child(1) .nav-btn',
    ...
    // 3. Проверка знаний — 12 Модулей
    {
      desktopTarget: '.nav-item:nth-child(1) .dropdown [href*="modules-index"]',
      mobileTarget: '.mob-acc:nth-child(1) .mob-acc-body [href*="modules-index"]',
      title: '✍️ Проверка знаний — 12 Модулей',
    ...
    // 4. AI Переговоры
    {
      desktopTarget: '.nav-item:nth-child(2) .dropdown [href*="ai-broker-chat"]',
      mobileTarget: '.mob-acc:nth-child(2) .mob-acc-body [href*="ai-broker-chat"]',
      title: '🤖 AI Переговоры',
    ...
    // 5. Тестирование
    {
      ...
      mobileTarget: '.mob-acc:nth-child(2) .mob-acc-body [href*="testing"]',
      title: '✍️ Проверить себя',
    ...
    // 6. Флеш-карточки
    {
      ...
      mobileTarget: '.mob-acc:nth-child(2) .mob-acc-body [href*="Trainer-Quiz"]',
      title: '💡 Флеш-карточки',
    ...
    // 7. Поиск грузов
    {
      ...
      mobileTarget: '.mob-acc:nth-child(2) .mob-acc-body [href*="load-finder"]',
      title: '📦 Поиск грузов',
    ...
    // 8. Система XP
    {
      desktopTarget: '#nav-actions-desktop, .nav-actions',
      mobileTarget: '#mob-xp-wrap, .mob-xp-wrap, .nav-actions',
      title: '⚡ Система XP',
    ...
    // 9. Финал — по центру
    {
      ...
      text: 'Ты готов! Открой первый урок и начни обучение. Удачи!',
      position: 'center',
      prepare: function () {
        if (isMobile()) { closeMobMenu(); }
        else { closeAllDropdowns(); }
  ...
  // ── CSS ───────────────────────────────────────────────────────
  var CSS = '\
    #onb-backdrop{position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:999991;pointer-events:all}\
    #onb-highlight{position:fixed;z-index:999994;border-radius:8px;box-shadow:0 0 0 3px rgba(6,182,212,0.9),0 0 0 9999px rgba(0,0,0,0.55);pointer-events:none;transition:all .3s cubic-bezier(.4,0,.2,1)}\
    #onb-tooltip{position:fixed;z-index:999995;background:rgba(15,23,42,0.97);backdrop-filter:blur(20px);border:1px solid rgba(6,182,212,0.35);border-radius:14px;padding:14px 16px;max-width:280px;min-width:200px;box-shadow:0 8px 40px rgba(0,0,0,0.5);pointer-events:all;transition:opacity .25s,transform .25s}\
    ...
    #onb-tooltip.pos-mob-bottom::before{display:none}\
    ...
    #onb-skip{padding:5px 10px;font-size:11px;font-weight:600;color:#64748b;background:transparent;border:none;cursor:pointer;transition:all .2s;margin-right:auto}\
    ...
    #mobMenu.active{z-index:999993!important}\
    @media(max-width:768px){\
      #onb-tooltip{padding:16px 18px;max-width:calc(100vw - 24px);min-width:0;width:calc(100vw - 24px);border-radius:16px;left:12px!important;right:12px!important;transform:none!important}\
      #onb-tooltip.pos-mob-bottom{bottom:16px!important;top:auto!important}\
      #onb-tooltip.pos-center{top:50%!important;left:50%!important;width:calc(100vw - 48px);transform:translate(-50%,-50%)!important}\
      #onb-step-badge{font-size:11px;margin-bottom:6px}\
      #onb-title{font-size:16px;margin-bottom:6px}\
      #onb-text{font-size:13px;line-height:1.55;margin-bottom:14px}\
      .onb-dot{width:6px;height:6px}\
      .onb-dot.active{width:18px;border-radius:4px}\
      #onb-dots{gap:5px;margin-bottom:2px}\
      #onb-back{padding:8px 14px;font-size:13px;border-radius:10px}\
      #onb-next{padding:8px 18px;font-size:13px;border-radius:10px}\
      #onb-skip{font-size:12px;padding:8px 4px}\
      #onb-actions{gap:8px}\
    }\
  ';
  ...
  function init() {
    ...
    tooltipEl.innerHTML = '<div id="onb-step-badge"></div><div id="onb-title"></div><div id="onb-text"></div><div id="onb-footer"><div id="onb-dots"></div><div id="onb-actions"><button id="onb-skip">Пропустить</button><button id="onb-back">\u2190 Назад</button><button id="onb-next">Далее \u2192</button></div></div>';
  ...
  // Найти видимый элемент по селектору
  function findTarget(selector) {
    if (!selector) return null;
    var parts = selector.split(', ');
    // Сначала ищем видимый
    for (var i = 0; i < parts.length; i++) {
      var el = document.querySelector(parts[i].trim());
      if (el && el.offsetParent !== null) return el;
    }
    // Fallback: любой найденный
  ...
  // Скроллим элемент внутри mob-menu чтобы он был виден
  function scrollMobMenuToTarget(target, cb) {
    var menu = document.getElementById('mobMenu');
    if (!menu || !menu.contains(target)) { cb(); return; }
    var menuRect = menu.getBoundingClientRect();
    var targetRect = target.getBoundingClientRect();
    // Если элемент уже виден в пределах меню — не скроллим
    if (targetRect.top >= menuRect.top && targetRect.bottom <= menuRect.bottom) {
      cb(); return;
    }
    menu.scrollTop += targetRect.top - menuRect.top - menuRect.height / 2 + targetRect.height / 2;
    setTimeout(cb, 350);
  }
  ...
  function showStep(index) {
    ...
    // Задержка: дать время открыться меню/accordion
    var delay = (typeof step.prepare === 'function') ? 350 : 100;
    setTimeout(function () {
      ...
      if (isMobile()) {
        // На мобильном: скроллим меню к элементу, потом позиционируем
        scrollMobMenuToTarget(target, function () {
          ...
          highlightEl.style.cssText = [
            ...
            'border-radius:8px',
            'box-shadow:0 0 0 3px rgba(6,182,212,0.9),0 0 0 9999px rgba(0,0,0,0.55)',
            'pointer-events:none',
            'transition:all .3s cubic-bezier(.4,0,.2,1)',
            'z-index:999994',
            'display:block'
          ].join(';');
          // Tooltip всегда снизу экрана на мобильном
          mobBottomTooltip();
        });
      } else {
        var rect = target.getBoundingClientRect();
        ...
        highlightEl.style.cssText = [
          ...
          'height:' + (rect.height + pad * 2) + 'px',
          'border-radius:10px',
          'box-shadow:0 0 0 4px rgba(6,182,212,0.8),0 0 0 9999px rgba(0,0,0,0.6)',
          'pointer-events:none',
  ...
  // Desktop: tooltip рядом с элементом
  function positionTooltip(targetRect, position) {
    tooltipEl.className = 'pos-' + position;
    tooltipEl.style.cssText = 'position:fixed;display:block;top:0;left:0;transform:none';
    ...
    var top = position === 'bottom' ? targetRect.bottom + gap : targetRect.top - th - gap;
  ...
  // Mobile: tooltip фиксирован снизу экрана
  function mobBottomTooltip() {
    tooltipEl.className = 'pos-mob-bottom';
    tooltipEl.style.cssText = 'position:fixed;display:block;bottom:16px;left:12px;right:12px;top:auto;transform:none;width:auto;max-width:calc(100vw - 24px)';
  }

  function centerTooltip() {
    tooltipEl.className = 'pos-center';
    tooltipEl.style.cssText = 'position:fixed;display:block;top:50%;left:50%;transform:translate(-50%,-50%)';
  }
  ...
  function showStep(index) {
    ...
    // Задержка для DOM (больше если нужно открыть dropdown)
    var delay = (typeof step.prepare === 'function') ? 300 : 100;
    setTimeout(function () {
      var selector = isMobile() ? step.mobileTarget : step.desktopTarget;
      var target = findTarget(selector);
