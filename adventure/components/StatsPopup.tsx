...
export default function StatsPopup({ onClose }: { onClose: () => void }) {
  ...
  const stats = [
    { label: 'День', value: `${day}`, icon: '📅', color: Colors.primary },
    { label: 'Очки', value: `${score}`, icon: '⭐', color: '#ffd60a' },
    { label: 'Репутация', value: `${reputation}%`, icon: '🏆', color: reputation > 70 ? Colors.success : Colors.warning },
    { label: 'Траки', value: `${trucks.length}`, icon: '🚛', color: Colors.primary },
    { label: 'Доход', value: `$${totalEarned.toLocaleString()}`, icon: '💰', color: Colors.success },
    { label: 'Расходы', value: `-$${totalLost.toLocaleString()}`, icon: '📉', color: Colors.danger },
    { label: 'Прибыль', value: `${profit >= 0 ? '+' : ''}$${profit.toLocaleString()}`, icon: '💵', color: profit >= 0 ? Colors.success : Colors.danger },
    { label: 'На трак', value: `$${perTruck.toLocaleString()}`, icon: '📊', color: Colors.primary },
    { label: 'Доставки', value: `${totalDeliveries}`, icon: '📦', color: '#5ac8fa' },
    { label: 'Мили', value: totalMiles.toLocaleString(), icon: '🛣', color: Colors.purple },
    { label: 'Safety', value: `${avgSafety}%`, icon: '🛡', color: avgSafety >= 90 ? Colors.success : Colors.warning },
    { label: 'HOS нарушения', value: `${hosViolations}`, icon: '⚠️', color: hosViolations > 0 ? Colors.danger : Colors.success },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '1px solid rgba(48,209,88,0.2)', borderRadius: 20,
        boxShadow: '0 20px 60px rgba(0,0,0,0.6)', padding: '24px 20px',
        scrollbarWidth: 'none',
      } as any}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <span style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>📈 Статистика</span>
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#94a3b8', marginBottom: 8 }}>🚛 По тракам</div>
          {trucks.map(t => (
            <div key={t.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: '#64748b' }}>{t.driver}</div>
              </div>
              <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'right' } as any}>
              ...
              <div style={{
                ...
                <span style={{
                  ...
                  color: (t.safetyScore || 100) >= 90 ? Colors.success : Colors.warning,
