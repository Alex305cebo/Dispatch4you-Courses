...
const TruckCardOverlay = memo(function TruckCardOverlay({ onTruckClick, selectedTruckId }: Props) {
  ...
  return (
    <div className="truck-card-scroll" style={{
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' } as any}>
                <span style={{ fontSize: 13, fontWeight: 900, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 100 } as any}>
                  {driverName}
                </span>
                <div style={{ display: 'flex', gap: 3, flexShrink: 0 } as any}>
                  <span style={{ fontSize: 9, fontWeight: 700, color: '#38bdf8', border: '1px solid rgba(56,189,248,0.3)', borderRadius: 4, padding: '1px 4px' } as any}>TRK {truckNum}</span>
                  <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 4, padding: '1px 4px' } as any}>TRL {trailerNum}</span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 3 } as any}>
                ...
                {truck.destinationCity ? (
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } as any}>
                    {fromLabel}
                    <span style={{ color: '#475569', margin: '0 3px' } as any}>→</span>
                    <span style={{ color: '#38bdf8' } as any}>{toLabel}</span>
                  </span>
                ) : (
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8' } as any}>{fromLabel}</span>
                )}
              </div>
              ...
              {isMoving && (
                <div style={{ marginTop: 1 } as any}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 } as any}>
                    <span style={{ fontSize: 9, color: '#64748b', fontWeight: 600 } as any}>Прогресс рейса</span>
                    <span style={{ fontSize: 9, fontWeight: 800, color } as any}>{progressPct}%</span>
                  </div>
                  <div style={{ height: 4, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' } as any}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 1 } as any}>
                ...
                {truck.currentLoad ? (
                  <div style={{ marginLeft: 'auto', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 6, padding: '1px 7px' } as any}>
                    <span style={{ fontSize: 13, fontWeight: 900, color: '#4ade80' } as any}>${truck.currentLoad.agreedRate.toLocaleString()}</span>
                  </div>
                ) : (
                  <div style={{ marginLeft: 'auto', border: '1px solid rgba(148,163,184,0.12)', borderRadius: 6, padding: '1px 7px' } as any}>
      ...
      >
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(56,189,248,0.12)', border: '1.5px solid rgba(56,189,248,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#38bdf8' } as any}>+</div>
