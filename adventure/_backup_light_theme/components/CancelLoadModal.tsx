    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <span style={{ fontSize: 18, fontWeight: 900, color: '#111827' }}>⚠️ Отмена груза</span>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: '#374151' }}>TONU штраф (25%)</span>
            <span style={{ fontSize: 13, fontWeight: 800, color: Colors.danger }}>-${tonuFee.toLocaleString()}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 12, color: '#374151' }}>Репутация</span>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#6b7280', marginBottom: 8 }}>Причина:</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 } as any}>
            {reasons.map((r, i) => (
              <div key={i} onClick={() => setReason(r)} style={{
                padding: '8px 10px', borderRadius: 8, cursor: 'pointer',
                background: reason === r ? 'rgba(255,69,58,0.1)' : '#f9fafb',
                border: reason === r ? '1px solid rgba(255,69,58,0.3)' : '1px solid rgba(0,0,0,0.06)',
                fontSize: 12, color: reason === r ? '#111827' : '#6b7280', fontWeight: reason === r ? 700 : 500,
        ...
        {!confirmed ? (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => reason ? setConfirmed(true) : null} style={{
              ...
              background: reason ? 'rgba(255,69,58,0.15)' : '#f9fafb',
              border: `1px solid ${reason ? 'rgba(255,69,58,0.3)' : 'rgba(0,0,0,0.06)'}`,
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleCancel} style={{
              ...
              color: '#111827', fontSize: 14, fontWeight: 900,
            }}>Да, отменить!</button>
          </div>
        )}
      </div>
