...
https.get(url, (res) => {
  ...
  res.on('end', () => {
    ...
    geojson.features.forEach(f => {
      ...
      // Get 2-letter abbreviation from state name
      const abbrevMap = {
      ...
      const abbr = abbrevMap[name];
      if (!abbr) { console.log('No abbr for:', name); return; }
      if (abbr === 'MI') return; // Michigan handled separately
    ...
    // Michigan Lower Peninsula (from amCharts - accurate)
    ...
    fs.writeFileSync('states_publicamundi.js', lines.join('\n'), 'utf8');
