...
// Домашняя сеть разрешена намеренно: микрофон и звук проверяются с телефона
// по адресу вида http://192.168.x.x:5180/broker-call/ — см. CLAUDE.md.
$ALLOWED_HOSTS = array('dispatch4you.com', 'localhost', '127.0.0.1');
$PRIVATE_NET   = true;

// Раньше стояла звёздочка: ответ этого прокси могла читать страница любого
// сайта. Отдаём ровно тот источник, который сами признали своим.
...
// Сравнивается ХОСТ целиком. Прежняя проверка искала подстроку и пропускала
// любой адрес, где встретились нужные буквы, — включая
// https://dispatch4you.com.чужой-домен.ru и https://зло.example/?dispatch4you.com.
if (!d4y_origin_allowed($ALLOWED_HOSTS, $PRIVATE_NET) && $action !== 'health') {
  bc_fail(403, 'forbidden');
}
